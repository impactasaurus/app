...
const CatalogueQuestionnaire = (p: {cos: ICatalogueOS, onClick: () => void}): JSX.Element => {
...
const CatalogueInner = (p: IProps) => {
  ...
  const renderQuestionnaire = (q: ICatalogueOS) => {
    return <CatalogueQuestionnaire cos={q} onClick={navigateToQuestionnaire(q.outcomeset.id)} />;
  }

  const {t} = useTranslation();
  return (
    <List divided={true} relaxed={true} verticalAlign="middle" className="list">
      {renderArray(renderQuestionnaire, p.data.getCatalogueQuestionnaires || [])}
