...
const RecordsInner = (p: IProps) => {
  ...
  const renderRecords = (): JSX.Element => {
    ...
    const noCompleteMeetings =
      !Array.isArray(p.data.getMeetings) || p.data.getMeetings.length === 0;
    const noIncompleteMeetings =
      !Array.isArray(p.data.getIncompleteMeetings) ||
      p.data.getIncompleteMeetings.length === 0;
