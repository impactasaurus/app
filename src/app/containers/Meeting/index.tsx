...
class MeetingInner extends React.Component<IProps, IState> {
  ...
  public componentDidUpdate() {
    if (this.props.questions !== undefined && this.state.init === false) {
      ...
      this.setMeetingState(
        initialState(this.hasInstructions(), this.props.questions.length)
      );
    }
  }
  ...
  private completed() {
    ...
    if (this.props.isBeneficiary) {
      ...
      journey(
        this.props.setURL,
        this.props.data.getMeeting.beneficiary,
        this.props.data.getMeeting.outcomeSetID
      );
    }
  }
  ...
  private hasInstructions() {
    return (
      !isNullOrUndefined(this.props.questionnaire.instructions) &&
      this.props.questionnaire.instructions.length !== 0
    );
  }

  private currentQuestionIdx(): number {
    return this.props.questions.findIndex(
      (q) => q.id === this.state.currentQuestion
    );
  }
  ...
  private renderProgressBar(): JSX.Element {
    ...
    if (
      this.state.screen === Screen.NOTES ||
      this.state.screen === Screen.REVIEW
    ) {
      value = this.props.questions.length;
    }
    return (
      <Progress value={value} total={this.props.questions.length} size="tiny" />
    );
  }
...
const MeetingWithData = getMeeting<IProps>((props) => props.match.params.id)(
  MeetingWithConnection
);
