...
const NewQuestionnaireTypSelectionInner = (p: IProps) => {
  ...
  const renderQuickStart = (data: IOutcomeResult) => {
    ...
    let body: JSX.Element|JSX.Element[] = <Button primary={true} onClick={quickStart}>{t("Add the ONS Wellbeing questionnaire")}</Button>;
    if(importing) {
    ...
    if(error) {
      body = [
        ...
        <p key="advice">{t("Please refresh and try again, if that doesn't work, please drop us an email at support@impactasaurus.org")}</p>,
      ];
    }
    return (
...
const NewQuestionnaireTypSelectionConnected = UrlHOC(NewQuestionnaireTypSelectionInner);
const NewQuestionnaireTypSelectionWithData =  allOutcomeSets(importQuestionnaire<IProps>(NewQuestionnaireTypSelectionConnected));

// t('New Questionnaire')
export const NewQuestionnaireTypeSelector = PageWrapperHoC('New Questionnaire', 'new-questionnaire', NewQuestionnaireTypSelectionWithData);
