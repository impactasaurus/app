...
import {
  INewQuestionnaire,
  NewQuestionnaireForm as NQF,
} from "components/NewQuestionnaireForm";
...
const NewQuestionnaireFormInner = (p: IOutcomeMutation & IURLConnector) => {
  ...
  const createQS = (q: INewQuestionnaire) => {
    return p.newQuestionSet(q.name, q.description).then(goToQuestionnaire);
  };

  return <NQF onCancel={goToTypeSelection} submit={createQS} />;
};

const NewQuestionnaireFormConnected = UrlHOC(NewQuestionnaireFormInner);

// t('Custom Questionnaire')
export const NewQuestionnaireForm = newQuestionSet(
  PageWrapperHoC(
    "Custom Questionnaire",
    "new-questionnaire-form",
    NewQuestionnaireFormConnected
  )
);
