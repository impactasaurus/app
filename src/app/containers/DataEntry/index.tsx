...
const DataEntryInner = (p: IProps) => {
  ...
  const completed = () => {
    p.setURL(`/beneficiary/${p.data.getMeeting.beneficiary}`, `?q=${p.data.getMeeting.outcomeSetID}`);
  }
  ...
  const renderQuestionPage = (): JSX.Element => {
    ...
    return (
      <div>
        ...
        <Button {...props} onClick={setPage(Screen.NOTES)}>{t("Next")}</Button>
      </div>
    );
  }
...
const DataEntryConnected = connect(propsPreprocessing, UrlConnector)(DataEntryInner);
// t("questionnaire")
const DataEntryLoader = ApolloLoaderHoC("questionnaire", (p: IProps) => p.data, DataEntryConnected);
const DataEntryWithData = completeMeeting<IProps>(getMeeting<IProps>((props) => props.match.params.id)(DataEntryLoader));
// t("Data Entry")
const DataEntry = MinimalPageWrapperHoC("Data Entry", "data-entry", DataEntryWithData);
export { DataEntry };
