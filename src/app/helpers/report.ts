...
function getQuestionWarnings(exclusions: IExclusion[], qs: IOutcomeSet): string[] {
  return exclusions.filter((e) => !isNullOrUndefined(e.question) && !isNullOrUndefined(e.beneficiary)).map((e) => {
...
export function getWarnings(exclusions: IExclusion[], qs: IOutcomeSet): string[] {
  let warns = [];
