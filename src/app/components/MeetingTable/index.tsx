...
const findMeeting = (
  ...
  return meetings.reduce(
    (last: IMeeting | undefined, m: IMeeting): IMeeting => {
      ...
      const mConducted = Date.parse(m.date);
      const lConducted = Date.parse(last.date);
...
const MeetingTable = (p: IProp): JSX.Element => {
  ...
  const getColumnTitle = (prefix: string, meeting: IMeeting): string => {
    return `${prefix} (${getHumanisedDate(new Date(meeting.date))})`;
  };
    ...
    return Array.from(meetings)
      ...
      .map((meeting) => {
        return {
          value: meeting.id,
          key: meeting.id,
          text: getHumanisedDate(new Date(meeting.date)),
        };
      });
  };
