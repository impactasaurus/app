...
export function MeetingRadarWithImpl<P extends ImplementationIProp>(RadarImpl: React.ComponentType<P>, innerProps?: P): React.ComponentType<IProp> {
  const MeetingRadar = (p: IProp) => {
    ...
    const preProcessData = (data: RadarData): RadarData => {
      ...
      data.series = data.series.map((x, i, d) => {
        ...
        if (shouldSeriesBeDeactivated(data) && i !== 0 && i !== d.length-1 && i !== d.length-2) {
          x.disabled = true;
        }
        return x;
      });
    ...
    const renderInfoMessage = (data: RadarData): JSX.Element => {
      ...
      return (
        <Message compact={true}>
          <Icon name="info" /> {t("By default, three records are shown on the radar chart. To view more, click the corresponding legend entries.")}
        </Message>
      );
    }
    ...
    const data = p.aggregation === Aggregation.CATEGORY ?
    ...
    return (
      <div className="meeting-radar">
        ...
        <RadarImpl data={preProcessData(data)} aggregation={p.aggregation} {...innerProps} />
      </div>
    );
  };
  return MeetingRadar;
