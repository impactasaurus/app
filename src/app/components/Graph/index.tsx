...
const Graph = (p: IProps): JSX.Element => {
  ...
  const wrapper = (inner: JSX.Element): JSX.Element => {
    return <div className="graph">{inner}</div>;
  };
  ...
  if (noRecords <= 1) {
    return wrapper(
      <Message info={true}>
        ...
        <Message.Content>
          {t(
            "The data contains less than two records, and as such, cannot be visualised as a line graph. Please select a different visualisation."
          )}
        </Message.Content>
      </Message>
    );
  }
  return wrapper(<Chart config={chartConfig(p.data)} />);
};
