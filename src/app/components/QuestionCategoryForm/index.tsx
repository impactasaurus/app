...
const InnerForm = (props: IProps & FormikProps<IQuestionCategory>) => {
  ...
  return (
    <Form className="question-category-form" onSubmit={submitForm}>
      ...
      <FormField
        error={errors.aggregation as string}
        touched={touched.aggregation}
        inputID="qcf-agg"
        label={t("Aggregation")}
        required={true}
      >
...
const QuestionCategoryFormInner = withFormik<IProps, IQuestionCategory>({
  ...
  handleSubmit: (
    v: FormikValues,
    formikBag: FormikBag<IProps, IQuestionCategory>
  ): void => {
    formikBag.setSubmitting(true);
    formikBag.setStatus(undefined);
    formikBag.props
      .onSubmitButtonPress(v.name, v.aggregation, v.description)
      .then(() => {
...
export const QuestionCategoryForm = withTranslation()(
  QuestionCategoryFormInner
);
