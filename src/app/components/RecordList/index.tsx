...
const RecordListInner = (p: IProp) => {
  ...
  const renderActions = (r: IMeeting): JSX.Element[] => {
    ...
    actions.push((<ConfirmButton key="delete" onConfirm={del(r)} promptText={t("Are you sure you want to delete this record?")} buttonProps={{icon: 'delete', compact:true, size:'tiny'}} tooltip={t("Delete")} />));
    actions.push((<Popup key="edit" trigger={<Button onClick={edit(r)} icon="edit" compact={true} size="tiny" />} content={t("Edit")} />));
    if (r.incomplete) {
      actions.push((<Popup key="continue" trigger={<Button onClick={resume(r)} icon="arrow right" compact={true} size="tiny" />} content={t("Continue")} />));
    } else {
      actions.push((<Popup key="view" trigger={<Button onClick={view(r)} icon="eye" compact={true} size="tiny" />} content={t("View")} />));
    }
    return actions;
  }
  ...
  const renderRecord = (r: IMeeting): JSX.Element[] => {
    let incomplete = (<span/>);
    if (r.incomplete) {
      incomplete = (<Popup trigger={<Icon name="hourglass half"/>} content={t("Incomplete")} />);
    }
    return [(
      <Table.Row key={r.id}>
        <Table.Cell>
  ...
  return (
    <div id="record-list">
      <Table celled={true} striped={true} className="record-list-table">
        ...
        <Table.Body>
          {renderArrayForArray(renderRecord, sortMeetingsByConducted(p.meetings, false))}
        </Table.Body>
      </Table>
    </div>
  );
