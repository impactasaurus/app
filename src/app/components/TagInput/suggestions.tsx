...
import {
  suggestTags,
  ISuggestTagsResult,
  suggestQuestionnaireTags,
} from "apollo/modules/tags";
import { Tag } from "../Tag";
...
const filterAlreadySelectedTags = (
  suggested: string[],
  selected: string[]
): string[] => {
...
const TagSuggestions = (p: IProps) => {
  ...
  const renderTag = (tag: string): JSX.Element => (
    <Tag key={tag} onClick={addTag(tag)} icon="plus" tag={tag} />
  );
  ...
  const unselectedSuggestedTags = filterAlreadySelectedTags(
    p.data.suggestedTags,
    p.tags
  );
  ...
  return wrap(
    <span>{renderArray<string>(renderTag, unselectedSuggestedTags)}</span>
  );
};

const getID = (p: IProps) => p.id;
export const TagSuggestionsForBen = suggestTags<IProps>(getID)(TagSuggestions);
export const TagSuggestionsForQuestionnaire =
  suggestQuestionnaireTags<IProps>(getID)(TagSuggestions);
