...
import { TagSuggestionsForBen, TagSuggestionsForQuestionnaire } from './suggestions';
import './style.less';
...
const TagInputWithSuggestions = (p: IProps): JSX.Element => (
  <div className="record-tag-input-w-suggestions">
    ...
    />
    {p.children}
    {p.type === SuggestionSource.BENEFICIARY && <TagSuggestionsForBen onChange={p.onChange} id={p.id} tags={p.tags} />}
    {p.type === SuggestionSource.QUESTIONNAIRE && <TagSuggestionsForQuestionnaire onChange={p.onChange} id={p.id} tags={p.tags} />}
  </div>
);
...
export const TagInputWithQuestionnaireSuggestions = (p: IExtProps): JSX.Element =>
