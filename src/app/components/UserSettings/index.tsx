...
const getLanguageOptions = (t: (s: string) => string, current?: string): DropdownItemProps[]=> {
...
const InnerForm = (props: IProps & FormikProps<IFormOutput>) => {
  const { touched, status, errors, isSubmitting, submitForm, isValid, handleChange, handleBlur, values, setFieldValue, dirty, handleReset, t } = props;
  ...
  return (
    <Form className="screen" onSubmit={submitForm}>
      ...
      <FormField error={errors.language} touched={touched.language} inputID="usf-lang" label={t("Language")}>
        <Form.Dropdown id="usf-language" name="language" onChange={languageSelected} selection={true} value={values.language} options={languages} />
      </FormField>
...
export const UserSettingsInner = withFormik<IProps, IFormOutput>({
  ...
  handleSubmit: (v: FormikValues, formikBag: FormikBag<IProps, IFormOutput>): void => {
    ...
    formikBag.props.updateSelf(vals.name, !vals.subscribed, vals.language)
      .then(() => {
