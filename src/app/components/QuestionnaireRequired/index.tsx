...
export const QuestionnaireRequired = <P extends unknown>(
  WrappedComponent: React.ComponentType<P>
): React.ComponentClass<P & IProp> => {
  class Inner extends React.Component<P & IProp, null> {
    public render() {
      if (Array.isArray(this.props.data.allOutcomeSets)) {
        ...
        const noQuestions = this.props.data.allOutcomeSets.reduce(
          (max, q) => Math.max(max, getQuestions(q).length),
          0
        );
        if (noQuestions === 0) {
