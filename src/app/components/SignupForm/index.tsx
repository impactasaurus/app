...
const InnerForm = (props: IProps & FormikProps<IFormOutput>) => {
  ...
  return (
    <Form className="screen" onSubmit={submitForm}>
      ...
      <FormField
        error={errors.emailCopy as string}
        touched={touched.emailCopy}
        inputID="su-email-copy"
      ...
      {shouldCollectOrg(props) && (
        <FormField
          error={errors.organisation as string}
          touched={touched.organisation}
          inputID="su-organisation"
          label={t("Organisation's Name")}
          required={true}
          description={signupVsInvite}
        >
          <Input
            id="su-organisation"
            name="organisation"
            type="text"
            placeholder={t("Your Organisation's Name")}
            onChange={handleChange}
            onBlur={handleBlur}
      ...
      <Form.Group
        error={errors.policyAcceptance as string}
        style={{ marginTop: "3em" }}
      >
        <Form.Checkbox
          id="su-policy"
...
const SignupFormInner = withFormik<IProps, IFormOutput>({
  validate: (values: IFormOutput, p: IProps) => {
    ...
    if (values.policyAcceptance !== true) {
      errors.policyAcceptance = t(
        "To use Impactasaurus you must accept the terms of use and privacy policy"
      );
    }
    ...
    if (
      shouldCollectOrg(p) &&
      (!values.organisation || values.organisation.length === 0)
    ) {
  ...
  handleSubmit: (
    ...
    formikBag.props.onFormSubmit(v as IFormOutput).catch((e: Error) => {
