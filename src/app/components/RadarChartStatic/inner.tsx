...
const shape = (columns, opt) => (data, i) => {
  ...
  return <path key={i} {...opt.shapeProps(data)} d={d} />;
};
...
const scale = (opt, value, cols): JSX.Element[] => {
  ...
  const line = (startIdx, endIdx) => {
    const start = [
      polarToX(cols[startIdx].angle, distFromOrigin),
      polarToY(cols[startIdx].angle, distFromOrigin),
    ];
    const end = [
      polarToX(cols[endIdx].angle, distFromOrigin),
      polarToY(cols[endIdx].angle, distFromOrigin),
    ];
    return (
      <polyline
        key={`${startIdx}-${endIdx}-${value}`}
...
export const render = ({ columns, data, opt = {} }: IProp) => {
  ...
  const groups = [<g key="shape">{data.map(shape(columns, opt))}</g>];
  if (opt.captions) {
    groups.push(<g key="captions">{columns.map(caption(opt))}</g>);
  }
  if (opt.axes) {
