...
const getRows = (p: IProps): IRow[] => {
  ...
  return latestData.map((d) => {
    const label = p.category ? getCategoryFriendlyName(d.id, p.questionSet) : getQuestionFriendlyName(d.id, p.questionSet);
    return {
      name: label,
      value: d.latest
    };
...
export const StatusReportTable = (p: IProps): JSX.Element => {
  ...
  const handleSort = (col: Column): () => void => {
    return () => {
      if (col !== column) {
        ...
        const newDir = (direction === Direction.ASC) ? Direction.DESC : Direction.ASC;
        setDirection(newDir)
      }
    };
  }
  ...
  return (
    <Table striped={true} celled={true} sortable={true}>
      <Table.Header>
        <Table.Row>
          <Table.HeaderCell sorted={column === Column.NAME ? directionSpec(direction) : null}
                            onClick={handleSort(Column.NAME)}>{nameCol}</Table.HeaderCell>
          <Table.HeaderCell sorted={column === Column.VALUE ? directionSpec(direction) : null}
                            onClick={handleSort(Column.VALUE)}>{t('Value')}</Table.HeaderCell>
        </Table.Row>
      </Table.Header>
